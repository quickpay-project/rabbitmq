...
// ===== INSERT LOG =====
func insertConfirmorderthreeLog(
	queueName string,
	body []byte,
	...
	query := `
INSERT INTO confirmorderthree_logs 
...
// ===== ยิงไปทุก API =====
func sendToAllConfirmorderthreeAPIs(queueName string, originalBody []byte, headers map[string]interface{}, data []byte) {
	urls := []string{
		os.Getenv("CONFIRMORDER_URL_GROUP3"),
		//os.Getenv("CONFIRMORDER_URL_GROUP2"),
		//os.Getenv("CONFIRMORDER_URL_GROUP3"),
	}
	...
	for _, apiURL := range urls {
		...
		go func(url string) {
			...
			if err != nil {
				...
				_, _ = insertConfirmorderthreeLog(queueName, originalBody, headers, 0, "", "failed", url, err.Error())
				return
			}
			...
			_, _ = insertConfirmorderthreeLog(queueName, originalBody, headers, resp.StatusCode, string(respBytes), status, url, errMsg)
		}(apiURL)
	}
...
// ===== RPC CONSUMER =====
func (r *RabbitConfirmorderthreeMQ) ConsconfirmorderthreeRPC() {
	...
	workerCountStr := os.Getenv("CONFIRMORDER_LIMIT")
	workerCount, err := strconv.Atoi(workerCountStr)
	if err != nil {
		log.Fatalf("❌ invalid CONFIRMORDER_LIMIT: %v", err)
	}
	...
	for i := 0; i < workerCount; i++ {
		...
		go func(id int) {
			defer wg.Done()
			for d := range msgs {
				processConfirmorderthreeMessage(d, ch, r.QueueName)
			}
		}(i)
	}
...
func processConfirmorderthreeMessage(d amqp.Delivery, ch *amqp.Channel, queueName string) {
	headers := map[string]interface{}{}
	...
	sendToAllConfirmorderthreeAPIs(queueName, d.Body, headers, d.Body)

	// ✅ reply กลับไปยังต้นทาง (optional)
	if d.ReplyTo != "" {
		resp, _ := json.Marshal(map[string]interface{}{
			"status":  200,
			"message": "confirmorder 3 update requests sent",
		})
