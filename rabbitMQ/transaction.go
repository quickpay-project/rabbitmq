...
type RabbitTransactionMQ struct {
	Body      string
	QueueName string
...
func genCorrelationTransactionID() string {
	b := make([]byte, 16)
	rand.Read(b)
...
// ========== RPC CLIENT ==========
func (r *RabbitTransactionMQ) TransactionRPC() ([]byte, error) {
	conn, ch := ConnectMQ()
	...
	corrID := genCorrelationTransactionID()
