...
// ===== ยิงไปทุก API =====
func sendToAllConfirmorderAPIs(queueName string, originalBody []byte, headers map[string]interface{}, data []byte) {
	urls := []string{
		os.Getenv("CONFIRMORDER_URL_GROUP1"),
	...
	for _, apiURL := range urls {
		...
		go func(url string) {
			...
			if err != nil {
				...
				_, _ = insertConfirmorderLog(queueName, originalBody, headers, 0, "", "failed", url, err.Error())
				return
			}
			...
			_, _ = insertConfirmorderLog(queueName, originalBody, headers, resp.StatusCode, string(respBytes), status, url, errMsg)
		}(apiURL)
	}
...
// ===== RPC CONSUMER =====
func (r *RabbitConfirmorderMQ) ConsconfirmorderRPC() {
	...
	for i := 0; i < workerCount; i++ {
		...
		go func(id int) {
			defer wg.Done()
			for d := range msgs {
				processConfirmorderMessage(d, ch, r.QueueName)
			}
		}(i)
	}
...
func processConfirmorderMessage(d amqp.Delivery, ch *amqp.Channel, queueName string) {
	headers := map[string]interface{}{}
	...
	sendToAllConfirmorderAPIs(queueName, d.Body, headers, d.Body)

	// ✅ reply กลับไปยังต้นทาง (optional)
	if d.ReplyTo != "" {
		resp, _ := json.Marshal(map[string]interface{}{
			"status":  200,
			"message": "confirmorder update requests sent",
		})
