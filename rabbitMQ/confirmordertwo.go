...
type RabbitConfirmordertwoMQ struct {
	Body      string
	QueueName string
...
func genCorrelationConfirmordertwoID() string {
	b := make([]byte, 16)
	rand.Read(b)
...
// ========== RPC CLIENT ==========
func (r *RabbitConfirmordertwoMQ) ConfirmordertwoRPC() ([]byte, error) {
	conn, ch := ConnectMQ()
	...
	corrID := genCorrelationConfirmordertwoID()
