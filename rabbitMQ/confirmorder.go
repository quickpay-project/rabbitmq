...
type RabbitConfirmorderMQ struct {
	Body      string
	QueueName string
...
func genCorrelationConfirmorderID() string {
	b := make([]byte, 16)
	rand.Read(b)
...
// ========== RPC CLIENT ==========
func (r *RabbitConfirmorderMQ) ConfirmorderRPC() ([]byte, error) {
	conn, ch := ConnectMQ()
	...
	corrID := genCorrelationConfirmorderID()
