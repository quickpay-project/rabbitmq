...
type RabbitConfirmorderthreeMQ struct {
	Body      string
	QueueName string
...
func genCorrelationConfirmorderthreeID() string {
	b := make([]byte, 16)
	rand.Read(b)
...
// ========== RPC CLIENT ==========
func (r *RabbitConfirmorderthreeMQ) ConfirmorderthreeRPC() ([]byte, error) {
	conn, ch := ConnectMQ()
	...
	corrID := genCorrelationConfirmorderthreeID()
